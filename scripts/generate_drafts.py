"""
generate_drafts.py - 下書き生成 CLI

使い方:
  python scripts/generate_drafts.py --account-id night_scout --limit 5 --dry-run --mock-llm
  python scripts/generate_drafts.py --limit 3 --dry-run
"""
import argparse
import os
import sys
...
def main() -> None:
    parser = argparse.ArgumentParser(description="v2 下書き生成")
    parser.add_argument("--account-id", help="対象アカウントID（省略時は全アカウント）")
    parser.add_argument("--limit", type=int, default=5, help="生成件数（デフォルト: 5）")
