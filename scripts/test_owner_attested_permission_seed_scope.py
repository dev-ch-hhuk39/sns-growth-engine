...
checks = [
    ("registered media input platforms only", platforms <= {"threads", "youtube", "tiktok", "x"}),
    ("active liver youtube included", "src_lm_yt_cand_001" in ids),
    ("note excluded", "src_lm_note_cand_001" not in ids),
    ("registered X media input can be owner-approved", any(source_id.startswith("src_ns_x") for source_id in ids)),
    ("permission output remains Threads only", sample.get("allowed_platforms") == "threads"),
