#!/usr/bin/env python3
"""Schedules metadata discovery but keeps heavy media work dispatch-only."""
...
for name in ("media-growth-production.yml", "media-growth-production-night-scout.yml"):
    ...
    assert "github.event_name == 'schedule'" not in heavy
    assert "github.event_name == 'workflow_dispatch'" in heavy
