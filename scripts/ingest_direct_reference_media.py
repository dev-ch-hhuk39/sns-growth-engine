...
def download_with_ytdlp(url: str, path: Path) -> None:
    import yt_dlp
    opts = {"quiet": True, "noplaylist": True, "outtmpl": str(path.with_suffix(".%(ext)s")), "retries": 2, "socket_timeout": 45, "max_filesize": 300 * 1024 * 1024}
...
def main() -> int:
    ...
    try:
        ...
        update_media_row(client, args.source_post_media_id, {"download_status": "DOWNLOADED", "cloudinary_status": "UPLOADED", "cloudinary_public_id": public_id, "storage_url": storage_url, "content_hash": digest_text, "mime_type": mime, "last_error": "", "updated_at": datetime.now(timezone.utc).isoformat()})
