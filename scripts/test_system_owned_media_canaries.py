...
for account in ("night_scout", "liver_manager"):
    batch_id = f"fresh_test_{account}_all"
    specs = media_canaries.build_specs(
        account,
        Path("/tmp/system-owned-media-test"),
        batch_id=batch_id,
    )
    ...
    assert all(
        ...
        and item["canary_id"] == f"canary_{batch_id}_{account}_{item['kind']}"
        for item in specs
    )
