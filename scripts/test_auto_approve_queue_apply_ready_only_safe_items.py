...
class Fake:
    def __init__(self): self.updated=[]; self.logs=[]
...
def main() -> int:
    ...
    checks=[("only one updated", res["updated_queue_ids"]==["q_safe"]),("status READY", client.updated[0][1]["status"]=="READY"),("queue_approved log", client.logs[0]["operation"]=="queue_approved")]
