...
def main() -> int:
    ...
    ok = len(rows) == 3 and all(s.get("manual_only") is True and s.get("source_type") == "account" for s in rows)
    print(f"  {'PASS' if ok else 'FAIL'} TikTok account sources manual_only")
