...
def main() -> int:
    ...
    checks = [
        ...
        ("schedule requires persisted activation", all("Resolve autonomous execution mode" in text and "scheduled_publish_enabled" in text and "production_publish_activation_approved" in text for text in texts)),
        ("apply uses resolved gate", all("if: env.RUN_AUTONOMOUS_APPLY == 'true'" in text for text in texts)),
