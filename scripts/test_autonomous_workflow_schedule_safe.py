...
def main() -> int:
    ...
    checks = [
        ...
        ("schedule is prepare-only", all("github.event_name == 'schedule' || github.event.inputs.confirm_autonomous == 'true'" not in text for text in texts)),
        ("manual publish needs activation", all("production_publish_activation_approved" in text and "github.event_name == 'workflow_dispatch' && github.event.inputs.confirm_autonomous == 'true'" in text for text in texts)),
