...
- Required repository secret NAMES for Cloudinary/Gemini are present. First authorized deploy run `36112819390` stopped before switching the release because a strict physical-line-count assertion rejected a quoted multiline value; follow-up changes only serialization validation to verify required variable names, `bash -n`, and mode 0600. No release or cron was changed by that failed run.
...
1. Merge the minimal runtime-env validation follow-up after required CI.
2. Deploy merged main to the existing Xserver runtime and verify runtime revision, cron, service, disk and secret presence (presence only).
