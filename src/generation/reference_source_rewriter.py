...
def _account_rules(account_id: str) -> str:
    ...
    if account_id == "liver_manager":
        return (
            ...
            "SOURCEに忠実な範囲で、読者が次の配信で実行できる具体的な行動を最低1つ明示する。"
            "その行動は、まず何をするか・何を見直すか・何を決めるか・何を整えるか・何を試すかが"
            "読者に分かる表現にし、なぜ有効かの理由や因果を1文添える。"
            "SOURCEから具体的な行動を安全に導けない場合は一般論で補わず __SKIP_SOURCE__ を返す。"
        )
