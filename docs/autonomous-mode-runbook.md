...
- AUTO_READY promotion also uses batched/retried queue updates. If an old run failed in `auto_approve_queue.py` with Sheets write quota 429, deploy the batch update version before rerunning.
...
- Approved media slots first exhaust bounded rights-valid inventory. If none exists, the slot records `SKIPPED_POLICY` and remains media-only; it never falls back to text. Ambiguous publish results never trigger fallback or retry.
...
- Development/rollout acceptance requires merged CI, active automation, a bounded real canary per account, read-after-write, recovery, metric reservations and PDCA wiring. Three consecutive real `schedule` events plus 24h/72h/168h measured metrics remain separate long-term observation evidence; pending windows do not make development incomplete.
- GitHub may start a cron event after its target window. `Content Slot Recovery` therefore scans every 30 minutes and automatically applies at most one slot per account after it is 20 minutes overdue. It recovers text with a bounded safe text route and uses approved media only for media slots. A partial/ambiguous publish or missing metrics jobs fails the recovery run and is never retried as a new post.
