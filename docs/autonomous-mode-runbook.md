...
- GitHub may start a cron event after its target window. `Content Slot Recovery` therefore scans every 30 minutes and automatically applies at most one slot per account after it is 20 minutes overdue. It prefers approved READY media, otherwise uses an explicitly audited safe original-text fallback. A partial/ambiguous publish or missing metrics jobs fails the recovery run and is never retried as a new post.
