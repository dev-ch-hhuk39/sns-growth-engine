...
## 実際の導入ステータス（2026-06-11）

| ツール | 状態 | 登録方法 |
|--------|------|----------|
| context-mode v1.0.162 | ✓ MCP 登録済み・全 PASS | `claude mcp add context-mode --scope project -- npx -y context-mode` |
| CodeGraph v0.9.9 | ✓ MCP 登録済み・115ファイル indexed | `codegraph install --target claude --location local --yes` |
| Headroom v0.23.0 | ✓ 保持（補助枠） | `~/.venvs/headroom/` + `~/.local/bin/claude-hr` |
| Hermes Agent | 未インストール（将来枠） | — |
...
- Phase ENV-1 完了: context-mode MCP サーバー登録・doctor 全 PASS 確認（2026-06-11）
