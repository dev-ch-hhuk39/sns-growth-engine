...
- 生成投稿案は `WAITING_REVIEW`（metrics 由来は `DRAFT`）で書き込まれる。worker の `ELIGIBLE_STATUSES`={READY} には含まれないため、**生成しただけでは worker の投稿対象にならない**。投稿対象になるのは人間が `approve_queue.py` で `READY` に昇格させた行のみ。多層防御:
  1. 生成 CLI も委譲先も投稿処理を一切呼ばない（生成専用）。生成系は `READY` を直接書かない。
  2. worker は `READY` のみ拾う。`WAITING_REVIEW` / `DRAFT` / `PLANNED` は投稿対象外（項目ごとの人間 READY 昇格ゲート）。
  3. 実投稿は別経路 worker の三重ゲート（`--confirm-real-post` かつ `PUBLISH_ENABLED=true` かつ `ALLOW_REAL_THREADS_POST=true`）が必要。現状すべて禁止のため実投稿は不可能。
  4. `beauty_account` / X は本パイプラインで BLOCKED。
- 承認モデル: Threads worker も X の `publish_queue.py`（`--status READY` 必須）と同様、`READY` 必須の「項目ごとの人間 READY 昇格ゲート」を持つ。人間ゲートは `approve_queue.py`（WAITING_REVIEW → READY/REJECTED）。X と Threads で承認モデルは対称になった（旧「残存アーキ懸念」は解消済み）。状態遷移: `WAITING_REVIEW → READY → PROCESSING → POSTED`。
...
- 投稿案は `WAITING_REVIEW` で書き込まれる（本 CLI は生成専用で投稿しない）。worker は `READY` のみ拾うため `WAITING_REVIEW` のままでは投稿対象外。承認（`READY` 昇格）は `approve_queue.py` で人間が行う。詳細は「共通の安全設計」を参照。
...
- 第一防御は worker の `READY` 必須化。生成系は `READY` を直接書かず、人間が `approve_queue.py` で昇格させた行だけが投稿対象になる。
...
- Threads worker の `ELIGIBLE_STATUSES`={READY}。`WAITING_REVIEW` は直接拾わない。
  `approve_queue.py`（WAITING_REVIEW → READY/REJECTED）が「項目ごとの人間 READY 昇格ゲート」を担う。
...
- `approve_queue_required_for_ready`: `READY` 昇格は `approve_queue.py` 経由のみ
