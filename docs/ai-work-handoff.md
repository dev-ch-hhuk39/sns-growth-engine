...
- `SheetsClient.update_queue_item()` も `batch_update` + 429 retryに変更。Actions run `29177776988` の失敗原因は `auto_approve_queue.py` -> `SheetsClient.update_queue_item()` の旧 `update_cell` loop がSheets write quota 429を踏んだこと。
...
- `test_sheets_client_update_queue_item_batched.py`: PASS 5 / FAIL 0
