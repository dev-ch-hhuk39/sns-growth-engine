...
PR #301 merged to `93a53d4cc038ca8d0028640d515e17c9cfb0f6c4`, CI `34723218843` 906 scripts PASS. Night direct prep `34723217888` still had zero READY despite one ingested bundle. Local Whisper tiny produced noisy/inadequate content evidence; follow-up switches bounded direct/clip preparation to small with unchanged single-thread/int8 and duration/count limits. Stored non-blocked tiny evidence may refresh once under existing ingest gates; current-model outcomes remain terminal. Do not call this an accuracy improvement until live evidence confirms it. No manual post or buffered activation.
...
- PR: `#26` (UNMERGED)
- implementation validation CI run: 30144585103
...
# 2026-09-13 Continuation: Beauty Duplicate Preparation

PR #302 is merged at `3cb05a83544f83056a2848d4f2bc4145c1bb56f6`. Night preparation `34724866490` refreshed five stored assets using small ASR; READY stays zero due to source/media grounding. Do not count ingestion as publication. Beauty preparation schedule `34739383502` produced an already-posted fallback and was correctly rejected after Hybrid. The pending fix includes same-account public posted/active queue history in generation and checks both AI and static fallback with the unchanged approval duplicate guard before creating candidates. No foreign-account history or internal analysis enters its prompt; history read errors fail closed. No manual post or buffered activation. Liver small-model preparation `34739690753` transcribed one video but saved no usable clip evidence. Threads historical position 94 was incorrectly returning the newest five CLI posts; CLI/GraphQL now delegate unsupported historical positions to the existing bounded public-browser fallback, without expanding their five-post cap. This is a cursor correctness fix, not proof that historical public content is accessible.
# 2026-09-15 PR305 merge and production persistence

Normal merge main: `9854d9cffcc357ee91d689f2ead716640ebc9325`; exact-head full CI `34908686396` passed tests/dependency audit/secret history scan.
AUTO_READY run `34921321706` revealed `offline_original_read_after_write_failed:drafts`: Sheets USER_ENTERED turned string `0.0` into integer `0`. Preserve exact values with RAW writes and uncached literal reads in the offline-only path. 12 focused tests, 910 repository scripts PASS. Partial WAITING_REVIEW drafts remain preserved, not posted/reused as evidence.
Direct media run `34921582346` started without publish capability. Existing Xserver runner online; SSH identifies the SNS runner service under `/opt/github-runners/sns-growth-engine`, user `snsrunner`. External primary not yet installed; activation stays OFF until real reserve/cutover verification.
# 2026-09-25 Xserver Media Replenishment Production Closure

- Continue from `origin/main` `ecfddbd9440e4284623e33c2ed44ad944b62d0b2` on `fix/xserver-primary-media-replenishment`. This is reliability closure, not a redesign.
- Root cause: Xserver scheduled publisher/reconciler was active, but Xserver had no scheduled media-preparation invocation. GitHub's scheduled media workflow was delayed by hours, so it could not reliably replenish the daily media slots. Do not count manual workflow dispatch as natural replenishment proof.
- Readiness had two accounting defects: 72h text coverage counted media slots even though those slots must not fall back to text; media READY reserve was re-evaluated through extra semantic/quality checks rather than the existing publisher's actual dry-run contract. Fixes separate the coverage denominators and count distinct assets only when the real `process_one(..., dry_run=True)` returns `DRY_RUN`; publisher gates are unchanged.
- Added `run_buffered_media_preparation_host.py` and its shell entrypoint. The host runner serially invokes existing bounded direct prep for Night/Liver/Beauty and approved clip prep for Night/Liver (minimum three usable assets per route), checks resource/Cloudinary availability before each operation, uses a dedicated `flock`, masks all publisher/X gates, and never publishes. YouTube PO token provider cleanup runs in `finally` and only stops a process started by that invocation.
- Installer adds the media runtime requirements to immutable releases and a 05:15 JST Xserver preparation cron after Night's previous-day 25:00 recovery window and before the next 10:00 post. Existing per-account publisher crons are unchanged. GitHub media prep becomes explicit-confirmation manual fallback to avoid a delayed second timer.
- Deploy workflow carries the already-configured Cloudinary/Gemini runtime secrets into mode-0600 `runtime.env` and verifies required media dependencies; values are never printed. Missing/unavailable secrets or Cloudinary usage fail closed.
- Pre-change read-only production snapshot: disk 72.31%, prep permitted; evergreen valid bank Night/Liver/Beauty 50/30/50; actual publisher dry-run usable media Night Direct/Clip 3/3, Liver Direct/Clip 4/3, Beauty Direct 3. Current duplicate/unverified/missing-metrics were zero. Historical audit has 2 duplicate candidates and 64 unverified/missing-metrics records and is intentionally excluded from current acceptance, not mutated. Old text coverage output 58.33% was a denominator error; every text slot existed.
- Local focused contracts, Ruff fatal rules, compileall, YAML parse, shell syntax and diff check passed. Full repository regression (923 scripts) and required CI are pending; then merge/deploy and natural Xserver run evidence remain.
- Before claiming completion, prove the natural preparation run refills each publisher-usable route to minimum, check text coverage 100%, evergreen >=30/account, scheduler and recovery active, current duplicate/unverified zero, metrics reservation and PDCA status. Runtime secrets are presence-only; never print values. If Cloudinary usage is unavailable, do not upload or claim replenishment success.
- No post, cleanup, historical row repair, manual media upload, or X/Beauty gate change was performed. Keep `.runtime/` untracked and untouched.
- Post-merge deploy attempt `36112819390` failed in the runtime.env preflight before immutable release installation or cron replacement. Repository secret names were present. The failure was the physical line-count check rejecting a quoted newline-containing value, not a missing secret. Follow-up changes only replace that brittle count with expected assignment-name checks, `bash -n`, and mode-0600 verification. Redeploy after exact-head CI; confirm current release and cron are still unchanged until then.
- Deploy `36113687899` passed the repaired env serialization guard, then failed in the installer. Xserver's newly created Python 3.10 venv bootstraps pip 22.0.2; the media requirements install triggers pip's internal `get_topological_weights` `AssertionError`. The current release and crontab were not switched. The next patch pins pip 26.2.1 before dependency installation (official pip release supports CPython 3.10); keep all app dependency pins and runtime gates unchanged.
