...
- PR #346 merged normally to `d97b57438be9a40640204b627c05ec5ff35d18b3`. First authorized deploy run `36112819390` stopped before release switch because the runtime.env line-count check rejected a safely quoted multiline secret value; required secret NAMES exist at repository scope. A minimal validation correction (variable names + `bash -n` + mode 0600) is being sent as a follow-up PR. No current release or cron was changed by the failed deploy.
