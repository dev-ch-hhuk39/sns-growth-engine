...
## Production Adoption Policy Table (2026-07-02)

| target | adopted tool(s) | purpose | status | production ON conditions | notes |
|---|---|---|---|---|---|
| TikTok video metadata | `yt-dlp` main, `tiktok-to-ytdlp` helper | metadata for individual TikTok `/video/` URLs | `yt-dlp`: installed/imported/wired/tested; `tiktok-to-ytdlp`: optional helper | human-provided real `/video/` URL, dry-run PASS, no rights violation | profile URLs are not expanded for download; third-party stays `download=false` and reference analysis only |
| TikTok trend research | `last30days-skill` | source discovery / trend signal | optional / external dependency; wired only if fetcher/runtime exists | human confirms execution method, query templates, output schema, and rate limits | do not use trend output to directly generate post body copy |
| X post research | Agent Reach, `last30days-skill`, `tweepy` | source discovery / external signal / shortlist enrichment | Agent Reach optional; last30days optional; tweepy installed but X fetch/post OFF | human confirms install source, ToS/session policy, API budget; explicit dry-run only | no X media save/repost; X fetch/post remains disabled by default |
| YouTube video metadata | `yt-dlp` | channel/video/playlist metadata dry-run | installed/imported/wired/tested | human-provided source URL, `download=false`, rights gates reviewed | third-party video download is prohibited; owned/licensed/approved creator only may enter media pipeline |
| YouTube transcript/research | `youtube-transcript-api`, Agent Reach, `last30days-skill` | public/official transcript, research signal | youtube-transcript-api installed/imported/wired/tested; Agent Reach/last30days optional | transcript available without video download; no preview/body leakage; optional tools approved by human | transcript text preview is suppressed; third-party stays analysis only |
| Threads collection | requests/stdlib HTTP, BeautifulSoup4/lxml, Playwright | public HTML/OG metadata and browser metrics fallback | requests installed/wired; BS4/lxml installed/imported/wired/tested; Playwright installed/imported/wired/tested | legal/ops reviewed URL set, no storage_state/cookie display, dry-run PASS | public metrics may be UNAVAILABLE; unknown values stay null; media is reference-only |
| video cut/media | ffmpeg CLI, ffmpeg-python, Cloudinary SDK, Pillow | approved clip planning, local media checks, future upload | ffmpeg-python/Cloudinary/Pillow installed; ffmpeg CLI optional/wired/tested | `rights_status` owned/licensed/approved_creator_clip, env + confirm gate, human review | third_party_reference_only/unknown are BLOCKED; no real cut/upload in inventory turns |
| optional/rejected tools | CLI-Anything, Threads scraper tools, TikTokApi, twikit, snscrape, MoneyPrinterTurbo, ViMax, Whisper/faster-whisper/PaddleOCR/VoxCPM | possible future or rejected tools | CLI-Anything optional/not_found; Threads scraper optional/rejected; TikTokApi optional/not_found; twikit/snscrape optional/not_found; MoneyPrinterTurbo/ViMax optional/rejected; heavy AI media tools optional/heavy/not installed | separate proposal, ToS review, dependency review, and tests | do not report these as production-ready in this repo |
