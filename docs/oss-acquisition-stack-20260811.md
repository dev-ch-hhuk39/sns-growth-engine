...
| Brainicism/bgutil-ytdlp-pot-provider `37169ee2656e08c5c2e5dc9df4c598c0cb4c88a8` | GPL-3.0-or-later | Version-matched 2.0.0 Python plugin plus immutable local-loopback Docker sidecar; no account cookie or API key | yt-dlp PO Token Provider Framework fallback for bounded approved YouTube media | Startup verifies the plugin version before accepting the provider health check | FALLBACK for physical approved YouTube media only |
| coletdjnz/yt-dlp-getpot-wpc `c75bca75ae94b07908ce62303eafeeb45bcbcec1` | MIT | Version-matched 1.1.2 Python plugin; approved Chrome, Xvfb, and Node 22 host runtimes; no account cookie or API key | Browser-minted GVS token fallback when the local bgutil provider cannot expose physical formats | Bounded live probe downloaded and ffprobe-validated an approved YouTube video with video and audio streams | SECONDARY FALLBACK for physical approved YouTube media only |
