...
| Inventory | Proactive next-24h Night/Liver maintenance plus bounded per-slot recovery; Beauty keeps its dedicated preparation | No operational `NO_READY_QUEUE` on expected text slots |
...
The capability matrix separates immediate development rollout from time-based production observation. CI and dry-run cannot fabricate live canary evidence, but an unelapsed 168-hour window is reported as observation in progress rather than unfinished development.
